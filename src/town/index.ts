...
const assignQuests = (openVillagers: CreepMemory[], openQuests: Quest[]) => {
  const matches = openVillagers.reduce((matches, villager) => {
    ...
    if (possibleQuests.length) matches[villager.name] = possibleQuests[0].id

    return matches
  }, {} as { [villagerId: string]: string })

  Object.keys(matches).forEach(villagerName => {
